...
# Only restore playit_claim.txt if missing
...
while true; do
    ...
    # Only save claim link if missing
    if [ ! -s links/playit_claim.txt ]; then
    ...
    # ------------------------------
    pkill tmate || true
    rm -f /tmp/tmate.sock          # <-- Remove old socket completely
    tmate -S /tmp/tmate.sock new-session -d
    tmate -S /tmp/tmate.sock wait tmate-ready 30 || true
    sleep 3

    # Ensure SSH link is not empty
    TMATE_SSH=""
    ...
    echo "[INFO] Updated tmate SSH: $TMATE_SSH"
